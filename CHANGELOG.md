...
- Added `ClaudeCodeLLM` and `CodexCLILLM`, two LLM providers backed by the agent CLIs already installed on the machine (Claude Code and Codex). Each one spawns its CLI in non-interactive mode (`claude --print`, `codex exec`), which signs in with the OAuth session created by `claude /login` / `codex login`, so a Claude or ChatGPT subscription can be used without an API key and without this library ever reading a token. The executable is auto-discovered on `PATH` and in the usual install directories (overridable with `CLAUDE_CLI_PATH` / `CODEX_CLI_PATH` or the `executable` argument), and the child process has `ANTHROPIC_API_KEY`/`OPENAI_API_KEY` removed by default so the stored OAuth session wins over a stray exported key (`use_oauth=False` to opt out). Both support `response_format` (passed to the CLI as a JSON schema) and are usable with `SimpleKGPipeline(..., use_structured_output=True)`. `ClaudeCodeLLM.auth_status()` / `CodexCLILLM.auth_status()` report on the credential store without exposing secrets, and the shared base class `BaseCLILLM` is exported for other CLI-backed providers.
